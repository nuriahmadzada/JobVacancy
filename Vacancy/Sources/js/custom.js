...
$(document).ready(function() {
    ...
    $("#contactForm").on('submit', function (e) {
        ...
        if (isValidEmail(data['email']) && (data['name'].length > 1) && (data['phone'].length > 1)) {
            $.ajax({
                url: "/contactus/message/",
                type: "POST",
                data: data,
                    success: function () {
                      $('#contactForm .input-success').delay(100).fadeIn(100);
                      $('#contactForm .input-error').fadeOut(100);
                    }
                });
                    } else {
                      $('#contactForm .input-error').delay(100).fadeIn(100);
    ...
    $(".subscribe").on('submit', function (e) {
        ...
        if (isValidEmail(data['email']) && data['email'].length != 0) {
            $.ajax({
                ...
                data: data,
                success: function () {
                    $('.subscribe .newsletter-success').delay(100).fadeIn(1000);
                    $('.subscribe .newsletter-error').fadeOut(1000);
                    $(".subscribe").trigger("reset");
                }
            });
        } else {
            $('.subscribe .newsletter-error').delay(100).fadeIn(1000);
            $('.subscribe .newsletter-error').fadeOut(1000);
            $(".subscribe").trigger("reset");
        }
        return false;
